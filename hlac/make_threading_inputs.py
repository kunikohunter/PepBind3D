...
<HOME>/main_project/scripts/slurm_batch_commands.sh.
...
1. Command template
   Source: <HOME>/main_project/scripts/slurm_batch_commands.sh
...
4. Allele string form: "<LETTER>*<NN>:<NN>" (e.g. "C*01:02"), IDENTICAL
   ...
   under <HOME>/Data/MHC_database/IEDB_data/ is a different,
...
5. FASTA header format for the batch files this stage produces:
   ...
   <HOME>/main_project/scripts/thread_IEDB_peptides_cluster.py
...
7. Command file path convention.
   ...
   (<CLUSTER>/hlac_generator/output/), a relative path
...
def main():
    ...
    parser.add_argument(
        "--csv",
        default="<HOME>/main_project/scripts/hla_c/hlac_threading_targets.csv",
